const Discord = require('discord.js');
exports.run = async (client, message, args) => {
  
  const settings = client.settings.get(message.guild.id);
  if (settings.moderation !== 'true') {
    message.channel.send("Moderation is disabled on this server")
    return;
  }
    var naughtyperson = message.mentions.members.first()
    if(!naughtyperson){
      var embed = new Discord.MessageEmbed()
      .setAuthor(message.author.username, message.author.displayAvatarURL())
      .setDescription("Please specify a user to be banned.")
      message.channel.send(embed)
        //message.reply("Tag someone to be banned.")
        return;
    }
    if(naughtyperson.hasPermission('MANAGE_MESSAGES')){
            var embed = new Discord.MessageEmbed()
      .setAuthor(message.author.username, message.author.displayAvatarURL())
      .setDescription("They can't be banned.")
      message.channel.send(embed)
        // message.channel.send('They cannot be banned.')
        return;
    }
    var reasonforban = args.slice(1).join(" ")
    if(!reasonforban){
                var embed = new Discord.MessageEmbed()
        .setAuthor(message.author.username, message.author.displayAvatarURL())
        .setDescription(`Please specify a reason for ${naughtyperson} to be banned.`)
        message.channel.send(embed)
        return;
    }
    await naughtyperson.ban(message.author.tag + reasonforban)
        .catch(error => message.reply(`Oh no! I can't ban him! My privellege must be too low. Try moving my role up! If that doesn't fix it, be sure to join the support server: https://discord.gg/WfPwbsS`));
  
  var embed = new Discord.MessageEmbed()
                .setAuthor(message.author.username, message.author.displayAvatarURL())
            .setDescription(`User has been banned.`)
            message.channel.send(embed)
  let modlog = message.guild.channels.find(channel => channel.name === settings.modlogchannel);
    var embed = new Discord.MessageEmbed()
  .setAuthor(message.author.tag, message.author.displayAvatarURL())
  .setDescription(`**Member**: ${naughtyperson} (${naughtyperson.id})\n**Action**: Ban\n**Reason**: ${reasonforban}`)
  .setTimestamp()
    .setFooter("ID:" + message.id)
  
  modlog.send(embed)
};

exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: [],
  permLevel: "Administrator"
};

exports.help = {
  name: 'ban',
  category: 'Moderation',
  description: 'Bans the mentioned user.',
  usage: 'ban [mention] [reason]'
};
