...
exports.run = (client, msg, args) => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(`**Nickname**: ${member.nickname !== null ? `${member.nickname}` : "None"}\n**Status**: ${status[member.user.presence.status]}\n**Joined Server**: ${moment.utc(member.joinedAt).format("dddd, MMMM Do YYYY, HH:mm:ss")}\n**Roles**: ${member.roles.filter(r => r.id !== msg.guild.id).map(roles => `\`${roles.name}\``).join(" | ")} | @everyone `)
  msg.channel.send({
    embed
  });
};
